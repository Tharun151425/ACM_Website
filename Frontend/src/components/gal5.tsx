...
const Gallery = () => {
  ...
  // Colors based on theme (from globals.css)
  const colors = {
    background: isDarkMode ? 'bg-[#000000]' : 'bg-[#ffffff]',
    foreground: isDarkMode ? 'text-[#e9ebec]' : 'text-[#141c23]',
    card: isDarkMode ? 'bg-[#19191c]' : 'bg-[#f7f8f8]',
    cardForeground: isDarkMode ? 'text-[#d9d9d9]' : 'text-[#141c23]',
    primary: 'bg-[#0384fa]',
    primaryForeground: 'text-white',
    secondary: isDarkMode ? 'bg-[#f2f8fa]' : 'bg-[#141c23]',
    secondaryForeground: isDarkMode ? 'text-[#141c23]' : 'text-white',
    accent: isDarkMode ? 'bg-[#091221]' : 'bg-[#e4eef9]',
    accentForeground: isDarkMode ? 'text-[#0384fa]' : 'text-[#0384fa]',
    muted: isDarkMode ? 'bg-[#181818]' : 'bg-[#e5e6e6]',
    mutedForeground: isDarkMode ? 'text-[#767676]' : 'text-[#141c23]',
    border: isDarkMode ? 'border-[#262626]' : 'border-[#e8eef3]',
  };
  ...
  return (
    <div className={`w-full ${colors.background} ${colors.foreground}`}>
      {/* Gallery Header Section */}
      <div className="max-w-7xl mx-auto px-4 py-12 text-center">
        <h1 className={`text-4xl font-bold mb-2 ${colors.foreground}`}>
          ACM RVCE GALLERY
        </h1>
        ...
        <div className="flex flex-col md:flex-row justify-center gap-4 mb-8">
          ...
          <div className="relative">
            <input
              ...
              className={`px-4 py-2 pl-10 rounded-full w-full md:w-64 ${isDarkMode ? colors.card : 'bg-white'} ${colors.border} border`}
            />
          ...
          <button 
            onClick={toggleTheme}
            className={`px-4 py-2 rounded-full font-medium ${colors.primary} ${colors.primaryForeground}`}
          >
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm transition-all duration-300 
                ${selectedCategory === category 
                  ...
                  : 'bg-transparent ' + colors.foreground + ' border'}`}
            >
              {category}
            </button>
          ))}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(9)].map((_, index) => (
              <div 
                key={index}
                className={`animate-pulse rounded-lg h-64 ${colors.muted}`}
              />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 opacity-0 animate-fadeIn">
            <AnimatePresence>
              {filteredItems.map(item => (
                <motion.div
                  ...
                  className={`${colors.card} rounded-lg overflow-hidden shadow-lg cursor-pointer group`}
                ...
                >
                  ...
                  <div className="relative overflow-hidden aspect-video">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent 
                      ...
                      <span 
                        className={`mt-2 inline-block px-2 py-1 text-xs font-medium rounded-full ${colors.primary} ${colors.primaryForeground}`}
                      >
                        {item.category}
                      </span>
                    </div>
                  ...
                  <div className="p-4">
                    <h3 className={`text-lg font-semibold mb-1 ${colors.cardForeground}`}>
                      {item.title}
                    </h3>
                    <p className={`text-sm opacity-75 ${colors.cardForeground}`}>
                      {item.date}
                    </p>
                  </div>
                </motion.div>
      ...
      {selectedImage && (
        ...
        >
          <motion.div 
            ...
            className={`relative max-w-6xl w-full rounded-xl overflow-hidden shadow-2xl ${colors.card}`}
          ...
          >
            <div className="flex flex-col md:flex-row">
              ...
              <div className={`md:w-1/4 p-6 flex flex-col justify-between ${colors.cardForeground}`}>
                <div>
                  ...
                  <div className="mt-4">
                    <span 
                      className={`inline-block px-3 py-1 rounded-full text-sm ${colors.primary} ${colors.primaryForeground}`}
                    >
